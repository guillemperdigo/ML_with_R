...
We also saw that in "train" there's a group of outliers at around RSSI = -20, not present in "valid"
```{r}
train %>%
  select(10:529) %>%
  max()
...
We'll filter out WAP's with maximum strength = -110 (it means we never receive any signal from them)
```{r}
max110_tr <-
  maxStr_train %>% 
  filter(V1 == -110)
...
There are 55 WAP's without reception (all values = -110) in train and 153 WAP's without reception in valid. Interestingly, they do not overlap. Let's exclude them all.
```{r}
